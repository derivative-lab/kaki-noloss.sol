...
contract ERC20 is Context, IERC20 {
    ...
    uint256 public interestRate = 500000;
    uint256 _base = 1000000000;
    uint256 fullBlock = 60; // 1week = 40320 15min = 60 1day1h = 6000 
    ...
    function getInterest(address account) public view returns(uint256) {
        ...
        uint256 delta = end.sub(start);
        return (_balances[account]).mul(interestRate).mul(delta).div(fullBlock).div(_base);
    }
    ...
    function updateInterest(address account) internal {
        ...
        _balances[account] = _balances[account].add(interest);

        _checkPorint[account] = getBlockNumber();
    }
    ...
    function balanceOf(address account) public view virtual override returns (uint256) {
        uint256 interest = getInterest(account);
        return (_balances[account]).add(interest);
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual { 
        if(from != address(0)) updateInterest(from);
        if(to != address(0)) updateInterest(to);
    }
}