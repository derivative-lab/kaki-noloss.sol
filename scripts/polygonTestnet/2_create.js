...
const kakinloAddress = require("../../.openzeppelin/unknown-80001.json").proxies[0].address;
...
async function main() {
  ...
  const KDAI = await ethers.getContractFactory("DAI");
  const kakinlo = await KakiNLO.attach(kakinloAddress);
  const kdai = await KDAI.attach(kERC20_L2_ADD_);

  const amount1 = "2020000000000000000000";
  await kdai.approve(kakinloAddress, amount1);
